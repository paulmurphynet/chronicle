...
- **Run tests and CI.** We have tests for the scorer, session flow, and verifier; CI includes ruff and pytest and is currently run manually (`workflow_dispatch`). Run `pytest tests/ -v` locally (after `pip install -e ".[dev]"`) and keep the suite green when you change code.
