...
The first three chapters described the problem, why it exists, and how we’re solving it. The fourth spelled out where challenges remain and what we don’t promise. This one is about **you**: how you can help make defensibility more practical and verifiable in real workflows. Chronicle is better when more people contribute—code, feedback, docs, or simply by spreading the story. Here are concrete ways to help.
...
- **Try the onboarding and interop harnesses.** Use [starter packs](../docs/starter-packs.md) for clean vertical bootstraps (journalism/legal/audit), then run the [integration export hardening harness](../docs/integration-export-hardening.md) to validate JSON/CSV/Markdown/`.chronicle`/signed-bundle interoperability artifacts.
- **Try Neo4j sync with deduplication.** If you use the graph pipeline ([aura-graph-pipeline](../docs/aura-graph-pipeline.md)), run `chronicle neo4j-sync --path <project> --dedupe-evidence-by-content-hash --report <path> --progress` (or set `NEO4J_DEDUPE_EVIDENCE_BY_CONTENT_HASH=1`) and report behavior at your scale.
- **Run live Neo4j integration tests.** If you have Neo4j available, run `CHRONICLE_RUN_NEO4J_LIVE_TESTS=1 CHRONICLE_EVENT_STORE=sqlite pytest tests/test_neo4j_live_integration.py -q` and share results.
- **Run tests and CI.** We have tests for scorer/session/verifier/API/interop paths; CI runs on push/PR/manual and includes core, frontend, and Postgres convergence gates. Run `pytest tests/ -v` locally (after `pip install -e ".[dev]"`) and keep the suite green when you change code.
- **Help close release blockers.** If you have repo-admin access, run the branch-protection rollout verifier (`scripts/check_branch_protection_rollout.py`) and share `reports/branch_protection_rollout_report.json` results.
...
We’re a small effort with a clear goal: make defensibility a practical, verifiable part of how people evaluate and trust answers. Every bit of help—code, docs, testing, or word of mouth—gets us closer. Thank you for reading, and for caring about “show your work.”
