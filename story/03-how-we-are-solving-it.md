...
## Friction tiers (Spark → Forge → Vault)

Investigations can move through **friction tiers** (e.g. Spark → Forge → Vault). Each tier can require more structure—typed claims, resolved tensions, source registration—before export or publication. The Reference UI and API support setting tier and viewing tier history, so teams can enforce “show your work” step by step. See [human-in-the-loop and attestation](../docs/human-in-the-loop-and-attestation.md) and [Reference UI plan](../docs/reference-ui-plan.md).
...
- **Human curation and attestation.** Every event records **who** did it (actor_id, actor_type). When a human (or tool) curates data—e.g. transcript ingestion, manual links—you can set your identity (CLI: **CHRONICLE_ACTOR_ID** / **--actor-id**; API: **X-Actor-Id** header) so the ledger attributes writes to you. Optionally, a **verification level** (e.g. from an IdP) can be stored so "attested with verified credential" is queryable. A [human-in-the-loop doc](../docs/human-in-the-loop-and-attestation.md) describes Propose–Confirm and dismissal-as-data. The **Reference UI** (in **frontend/**: React + Vite, API-only) offers **Try sample** (one-click minimal investigation), investigations list and detail (evidence, claims, links, defensibility, tensions, tension suggestions—**confirm** by declaring the tension, **dismiss** via the dismiss endpoint—export), and **Learn** guides per vertical (journalism, legal, compliance). A minimal **curation UI** (/static/curation.html with the API running) is also available.
- **Standards profile and publication path.** We now maintain explicit interoperability profiles (JSON-LD/PROV, ClaimReview, RO-Crate, plus C2PA and VC/Data Integrity compatibility paths) and a reproducible whitepaper/evidence-pack workflow for standards-facing publication. That means integrators can consume Chronicle through recognized standards layers without replacing the canonical `.chronicle` + verifier trust surface.
