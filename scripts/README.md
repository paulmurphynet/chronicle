...
| **benchmark_data/run_neo4j_projection_benchmark.py** | Benchmark Neo4j projection paths (CSV export and optional direct sync), with throughput/memory thresholds and JSON report output. | `PYTHONPATH=. python3 scripts/benchmark_data/run_neo4j_projection_benchmark.py --output reports/neo4j_projection_benchmark.json` |
...
| **benchmark_data/** | Benchmark sample generation and benchmark runners: `generate_benchmark_samples.py`, `generate_vertical_corpora.py`, `synthetic_training_pipeline.py`, `evals_to_preference_pair.py`, plus first-class runners `run_defensibility_benchmark.py` and `run_neo4j_projection_benchmark.py`. |
| **adapters/** | Map external formats to Chronicle: [example_rag_to_scorer](adapters/example_rag_to_scorer.py), [starter_batch_to_scorer](adapters/starter_batch_to_scorer.py), [ragas_batch_to_chronicle](adapters/ragas_batch_to_chronicle.py), [validate_adapter_outputs](adapters/validate_adapter_outputs.py), [fact_checker_to_chronicle](adapters/fact_checker_to_chronicle.py), [provenance_to_chronicle](adapters/provenance_to_chronicle.py). See [adapters/README.md](adapters/README.md). |
...
| **check_neo4j_ci_rollout.py** | Verify push/PR CI evidence for the `neo4j-live-integration` job via GitHub API; emits `reports/neo4j_live_ci_report.json`. |
| **check_post_public_finalization.py** | Aggregate post-public evidence artifacts (branch protection report, Neo4j CI report, standards dispatch log) into one gate report. |
