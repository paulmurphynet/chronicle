...
| **starter_batch_to_scorer.py** | Production-ready starter: read harness JSONL rows, map fields to eval contract, run scorer, emit one JSONL output row per run with `chronicle` payload. |
