...
| GET | `/investigations/{id}/policy-sensitivity` | R2-01 policy sensitivity report: compares the same investigation across selected profiles with claim-by-claim outcomes, pairwise deltas, and practical review implications. Query: `profile_id?` (repeat for multiple), `built_under_profile_id?`, `built_under_policy_version?`, `limit_claims?`. |
