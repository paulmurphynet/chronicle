...
def test_ensure_safe_http_url_accepts_http_and_https() -> None:
    assert (
        ensure_safe_http_url("https://example.com/x", block_private_hosts=False)
        == "https://example.com/x"
    )
    assert ensure_safe_http_url(
        "http://127.0.0.1:8000/health", block_private_hosts=False
    ).startswith("http://127.0.0.1:8000")
