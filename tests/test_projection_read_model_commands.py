...
def test_link_support_with_rationale_persisted(tmp_path: Path) -> None:
    ...
    with ChronicleSession(tmp_path) as session:
        ...
        _, ev_uid = session.ingest_evidence(
            inv_uid, b"Evidence text", "text/plain", original_filename="e.txt", actor_id="t", actor_type="tool"
        )
        _, span_uid = session.anchor_span(
            inv_uid, ev_uid, "text_offset", {"start_char": 0, "end_char": 12}, quote="Evidence text", actor_id="t", actor_type="tool"
        )
        _, claim_uid = session.propose_claim(inv_uid, "The claim.", actor_id="t", actor_type="tool")
        session.link_support(inv_uid, span_uid, claim_uid, rationale="This quote backs the claim.", actor_id="t", actor_type="tool")
...
def test_link_challenge_projection_and_defensibility_challenged(tmp_path: Path) -> None:
    ...
    with ChronicleSession(tmp_path) as session:
        ...
        _, ev_uid = session.ingest_evidence(inv_uid, b"X", "text/plain", original_filename="x.txt", actor_id="t", actor_type="tool")
        _, span_uid = session.anchor_span(inv_uid, ev_uid, "text_offset", {"start_char": 0, "end_char": 1}, quote="X", actor_id="t", actor_type="tool")
        ...
        _, ev2 = session.ingest_evidence(inv_uid, b"Not X", "text/plain", original_filename="y.txt", actor_id="t", actor_type="tool")
        _, span2 = session.anchor_span(inv_uid, ev2, "text_offset", {"start_char": 0, "end_char": 5}, quote="Not X", actor_id="t", actor_type="tool")
...
def test_list_claims_by_type_filter(tmp_path: Path) -> None:
    ...
    with ChronicleSession(tmp_path) as session:
        ...
        _, claim_uid_sef = session.propose_claim(inv_uid, "SEF claim.", initial_type="SEF", actor_id="t", actor_type="tool")
        ...
        sef_claims = read_model.list_claims_by_type(investigation_uid=inv_uid, claim_type="SEF", limit=10)
...
def test_list_claims_by_type_include_withdrawn(tmp_path: Path) -> None:
    ...
    with ChronicleSession(tmp_path) as session:
        ...
        _, claim_uid = session.propose_claim(inv_uid, "To withdraw.", actor_id="t", actor_type="tool")
        session.withdraw_claim(claim_uid, "Retracting.", actor_id="t", actor_type="tool", workspace="forge")
        read_model = session.store.get_read_model()
        with_withdrawn = read_model.list_claims_by_type(investigation_uid=inv_uid, limit=10, include_withdrawn=True)
        active_only = read_model.list_claims_by_type(investigation_uid=inv_uid, limit=10, include_withdrawn=False)
...
def test_list_claims_by_type_created_since(tmp_path: Path) -> None:
    ...
    with ChronicleSession(tmp_path) as session:
        ...
        recent = read_model.list_claims_by_type(investigation_uid=inv_uid, limit=10, created_since=created_at)
        future = read_model.list_claims_by_type(investigation_uid=inv_uid, limit=10, created_since="2099-01-01T00:00:00Z")
...
def test_get_defensibility_score_use_strength_weighting(tmp_path: Path) -> None:
    ...
    with ChronicleSession(tmp_path) as session:
        ...
        session.link_support(inv_uid, span_uid, claim_uid, strength=0.8, actor_id="t", actor_type="tool")
        read_model = session.store.get_read_model()
...
def test_get_defensibility_score_withdrawn_returns_none(tmp_path: Path) -> None:
    ...
    with ChronicleSession(tmp_path) as session:
        ...
        _, claim_uid = session.propose_claim(inv_uid, "Will withdraw.", actor_id="t", actor_type="tool")
        session.withdraw_claim(claim_uid, "Retracting.", actor_id="t", actor_type="tool", workspace="forge")
        read_model = session.store.get_read_model()
...
def test_list_evidence_by_investigation(tmp_path: Path) -> None:
    ...
    with ChronicleSession(tmp_path) as session:
        ...
        session.ingest_evidence(inv_uid, b"One", "text/plain", original_filename="a.txt", actor_id="t", actor_type="tool")
        session.ingest_evidence(inv_uid, b"Two", "text/plain", original_filename="b.txt", actor_id="t", actor_type="tool")
        read_model = session.store.get_read_model()
