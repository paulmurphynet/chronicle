...
def verify_append_only_ledger(conn: sqlite3.Connection, report: VerifyReport) -> None:
...
def verify_referential_integrity(conn: sqlite3.Connection, report: VerifyReport) -> None:
    ...
    for link_uid, claim_uid, span_uid in cur.fetchall():
        if conn.execute("SELECT 1 FROM claim WHERE claim_uid = ?", (claim_uid,)).fetchone() is None:
            failed.append(f"evidence_link {link_uid}: claim_uid {claim_uid} missing")
        if (
            conn.execute("SELECT 1 FROM evidence_span WHERE span_uid = ?", (span_uid,)).fetchone()
            is None
        ):
    ...
    for tid, a, b in cur.fetchall():
        if conn.execute("SELECT 1 FROM claim WHERE claim_uid = ?", (a,)).fetchone() is None:
            failed.append(f"tension {tid}: claim_a_uid {a} missing")
        if conn.execute("SELECT 1 FROM claim WHERE claim_uid = ?", (b,)).fetchone() is None:
    ...
    for table, col in [("claim_assertion", "claim_uid"), ("claim_decomposition", "claim_uid")]:
        try:
            ...
            for (uid,) in cur.fetchall():
                if (
                    conn.execute("SELECT 1 FROM claim WHERE claim_uid = ?", (uid,)).fetchone()
                    is None
                ):
    ...
    for cid, pid in cur.fetchall():
        if conn.execute("SELECT 1 FROM claim WHERE claim_uid = ?", (pid,)).fetchone() is None:
    ...
    for span_uid, ev_uid in cur.fetchall():
        if (
            conn.execute("SELECT 1 FROM evidence_item WHERE evidence_uid = ?", (ev_uid,)).fetchone()
            is None
        ):
    ...
    try:
        ...
        for cp_uid, art_uid in cur.fetchall():
            if (
                conn.execute(
                    "SELECT 1 FROM checkpoint WHERE checkpoint_uid = ?", (cp_uid,)
                ).fetchone()
                is None
            ):
                failed.append(f"checkpoint_artifact_freeze: checkpoint_uid {cp_uid} missing")
            if (
                conn.execute("SELECT 1 FROM artifact WHERE artifact_uid = ?", (art_uid,)).fetchone()
                is None
            ):
...
def verify_status_consistency(conn: sqlite3.Connection, report: VerifyReport) -> None:
...
def verify_projection_completeness(conn: sqlite3.Connection, report: VerifyReport) -> None:
...
def verify_evidence_integrity(
    conn: sqlite3.Connection, project_dir: Path, report: VerifyReport
) -> None:
